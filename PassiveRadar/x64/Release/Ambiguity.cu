...
int  Initialize(unsigned int BufferSize, unsigned int col, unsigned int row, float doopler_shift, short* name)
{
	...
	if (cufftPlan1d(&plan, (int)Nth, CUFFT_C2R, BATCH) != CUFFT_SUCCESS) {
		return CUDA_FFT_PLAN1D_CREATE_ERROR;
	}

	if (cufftPlan1d(&plan2, (int)Nth, CUFFT_C2R, Col) != CUFFT_SUCCESS) {
		return CUDA_FFT_PLAN1D_CREATE_ERROR;
	}
	...
	cudaMalloc(reinterpret_cast<void**>(&Cuda_bufW), N_corrected * Col);
	if ((su = cudaGetLastError()) != cudaSuccess) {
		return CUDA_MALLOC_ERROR - su;
	}
...
#pragma omp parallel for
	...
	for (int n = 0; n < Col; n++)
	{
		...
		//	return err;
			...
			//Output: Cuda_bufZ
		//err = CalcCorelateShift((size_t)( n));
		//if (err < CUDA_OK)
		//	return err;
	}
		//Painful part
		...
		//Input: Cuda_bufZ
		//Output: Cuda_bufZ
	/*	err = Magnitude(shift_cor, scale_type, 0);
		if (err < CUDA_OK)
			return err;*/
...
{
	//Backward fft
	if (cufftExecC2R(plan2, Cuda_bufZ, Cuda_bufW ) != CUFFT_SUCCESS) 
	{
		return CUDA_FFT_EXECUTE_ERROR;
	}
...
int Magnitude(int shift, short scale_Type, int Col_index)
{
	...
	MagnitudeCUDA << < numBlocks, threadsPerBlock >> > (Cuda_bufW, Cuda_ColRow, Row, Col_index * Row, shift, Nth);//
...
int CalcCorelateShift(size_t n)
{
	CorelateShiftCUDA << <blocksPerGrid, threadsPerBlock >> > ( Cuda_bufX, Cuda_bufY,Cuda_bufZ, Doppler_zoom_Col * n, n, Nth);
 ...
 *
 */
__global__ void CorelateShiftCUDA(cufftComplex* BufX, cufftComplex* BufY, cufftComplex* BufZ, size_t shift, size_t n_col, size_t numElements) {

	size_t i = blockDim.x * blockIdx.x + threadIdx.x;
	size_t j = i + shift;
	size_t k = i + numElements * n_col;
	//Just rotate from end to the beginning and convolute
	if (j < numElements) //protection
	{
		BufZ[k].x = BufY[i].x * BufX[j].x + BufY[i].y * BufX[j].y;
		BufZ[k].y = BufY[i].y * BufX[j].x - BufY[i].x * BufX[j].y;
	}
	else
	{
	//	if (i < numElements) //protection
		{
			...
			BufZ[k].x = BufY[i].x * BufX[l].x + BufY[i].y * BufX[l].y;
			BufZ[k].y = BufY[i].y * BufX[l].x - BufY[i].x * BufX[l].y;
		}
	}
}
 ...
 */
__global__ void MagnitudeCUDA(float* Inp, float* Out, int cuda_row, int col_index, int cuda_shift,  size_t numElements)
{
	int i = threadIdx.x;
	int c = threadIdx.y;

	 
	int j = i + cuda_shift + numElements * c;
	int k = i + cuda_row * c;
